...
# coolours for content.marketing
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
# filtering our all Apps that do not have a type
df %>% 
  filter(type =='NaN')
```
```{r echo=FALSE, warning=FALSE, message=FALSE}
# dropping our all Apps that do not have a type
df <- df %>% 
  filter(!type =='NaN')
```
...
There are 1637 Apps with 'VARIES WITH DEVICE'.
```{r echo=FALSE, warning=FALSE, message=FALSE}
df %>% 
  select(size) %>% 
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
df$catedfgory <- factor(df$category,levels = c( "Art & Design",
                                                  "Auto & Vehicles",
                                                  "Books & Reference",
                                                  "Business",
                                                  "Communication",
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
# filtering our all Apps that do not have a rating
df <- df %>% 
  filter(rating != 'NaN')
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
paid_df$catedfgory <- factor(paid_df$category,levels = c( "Art & Design",
                                                  "Auto & Vehicles",
                                                  "Books & Reference",
                                                  "Business",
                                                  "Communication",
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
free_df$catedfgory <- factor(free_df$category,levels = c( "Art & Design",
                                                  "Auto & Vehicles",
                                                  "Books & Reference",
                                                  "Business",
                                                  "Communication",
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
paid_box <- plot_ly(paid_df, y = ~category, x = ~rating) %>%
...
free_box <- plot_ly(free_df, y = ~category, x = ~rating) %>%
  add_boxplot(color = ~category) %>%
  layout(yaxis = list(title = FALSE),
         xaxis = list(title = FALSE))%>% 
  hide_legend()
...
fig <- subplot(paid_box, free_box, shareY= TRUE) %>% 
  layout(title = 'What categories have the most downloads?', annotations = annotations)
fig
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
paid_content_df <-  paid_df %>%
  select(category,installs_numeric) %>%
  group_by(category) %>%
  summarise(paid_sum_installs = sum(installs_numeric))

free_content_df <-  free_df %>%
  select(category, installs_numeric) %>%
  group_by(category) %>%
  summarise(free_sum_installs = sum(installs_numeric)) 


content_df <- merge(paid_content_df, free_content_df)

content_df$category <- factor(content_df$category,
                                      levels = c( "Art & Design",
                                                  "Auto & Vehicles",
                                                  "Books & Reference",
                                                  "Business",
...
paid_graph <- content_df %>%
  plot_ly(y = ~category, x = ~paid_sum_installs,  type = 'bar', name = 'Paid',   
          marker = list(color = factor(content_df$category,labels=c('#7F7F7F',  '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F',# 28, 
                                                    '#7F7F7F',  '#7F7F7F', '#7F7F7F', '#D62728', '#7F7F7F',# 18, 
                                                    '#7F7F7F',  '#D62728', '#7F7F7F', '#7F7F7F', '#7F7F7F',# 13, 
                                                    '#7F7F7F',  '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F',# 8, 
                                                    '#7F7F7F',  '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F',# 5
                                                    '#7F7F7F','#7F7F7F', '#7F7F7F'))),  
          hovertemplate = ~paste("Category:", category, "<br>",
                                 "Type: Paid", "<br>",
                                 "Number of Downloads:", paid_sum_installs)) %>%

  layout(yaxis = list(title = FALSE),
         xaxis = list(title = FALSE),
         showlegend = FALSE)

free_graph <- content_df %>% 
  plot_ly(y = ~category, x = ~free_sum_installs, type = 'bar', name = 'Free',
          marker = list(color = factor(content_df$category,labels=c('#7F7F7F',  '#7F7F7F', '#7F7F7F', '#7F7F7F', '#D62728',# 28, 
                                                    '#7F7F7F',  '#7F7F7F', '#7F7F7F', '#D62728', '#7F7F7F',# 23, 
                                                    '#7F7F7F',  '#D62728', '#7F7F7F', '#7F7F7F', '#7F7F7F',# 18, 
                                                    '#7F7F7F',  '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F',# 13, 
                                                    '#D62728',  '#7F7F7F', '#D62728', '#7F7F7F', '#D62728',# 8
                                                    '#7F7F7F','#7F7F7F', '#7F7F7F'))), # D62728
 
          hovertemplate = ~paste("Category:", category, "<br>",
                                 "Type: Free", "<br>",
                                 "Number of Downloads:", free_sum_installs)) %>%
...
fig <- subplot(paid_graph, free_graph, shareY= TRUE) %>% 
  layout(title = 'What categories have the most downloads?', annotations = annotations)
fig
```
...
## Does size of the price affect rating?
```{r echo=FALSE, warning=FALSE, message=FALSE}
d <- plot_ly(paid_df, x = ~rating, y = ~price, 
             mode = "markers", color = ~category #, size = counts, opacity = counts
             ) 
d
```

## Is there a higher percentage of paid apps in some categories than free apps? 
```{r echo=FALSE, warning=FALSE, message=FALSE}
paid_content_df <-  paid_df %>%
  select(category) %>%
  group_by(category) %>%
  summarise(paid_no_apps = n()) %>%
  mutate(paid_freq = round(paid_no_apps / sum(paid_no_apps), 2))
  # arrange(desc(paid_no_apps))

free_content_df <-  free_df %>%
  select(category) %>%
  group_by(category) %>%
  summarise(free_no_apps = n()) %>%
  mutate(free_freq = round(free_no_apps / sum(free_no_apps), 2)) 
#   arrange(desc(free_no_apps))
...
paid_graph <- content_df %>%
  plot_ly(y = ~category, x = ~paid_freq*100,  type = 'bar',   
          marker = list(color = factor(content_df$category,labels=c('#7F7F7F',  '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F',# 28, 
                                                    '#7F7F7F',  '#7F7F7F', '#7F7F7F', '#D62728', '#7F7F7F',# 18, 
                                                    '#7F7F7F',  '#D62728', '#7F7F7F', '#7F7F7F', '#7F7F7F',# 13, 
                                                    '#D62728',  '#7F7F7F', '#7F7F7F', '#D62728', '#7F7F7F',# 8, 
                                                    '#7F7F7F',  '#7F7F7F', '#7F7F7F', '#7F7F7F', '#D62728',# 5
                                                    '#7F7F7F','#7F7F7F', '#7F7F7F'))),  
          hovertemplate = ~paste("Category:", category, "<br>",
                                 "Type: Paid", "<br>",
                                 "Number of Category:", paid_no_apps, "<br>",
                                 "Percentage:", formattable::percent(paid_freq))) %>%

  layout(yaxis = list(title = FALSE),
         xaxis = list(title = FALSE, ticksuffix = "%",  range = c(0, 25)),
         showlegend = FALSE)

free_graph <- content_df %>% 
  plot_ly(y = ~category, x = ~free_freq*100, type = 'bar',
          marker = list(color = factor(content_df$category,labels=c('#7F7F7F',  '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F',# 28, 
                                                    '#7F7F7F',  '#7F7F7F', '#7F7F7F', '#D62728', '#7F7F7F',# 23, 
                                                    '#7F7F7F',  '#D62728', '#7F7F7F', '#7F7F7F', '#7F7F7F',# 18, 
                                                    '#7F7F7F',  '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F',# 13, 
                                                    '#7F7F7F',  '#7F7F7F', '#7F7F7F', '#7F7F7F', '#D62728',# 8
                                                    '#7F7F7F','#7F7F7F', '#7F7F7F'))), # D62728
 
          hovertemplate = ~paste("Category:", category, "<br>",
                                 "Type: Free", "<br>",
                                 "Number of Category:", free_no_apps, "<br>", 
                                 "Percentage:", formattable::percent(free_freq))) %>%
           
  layout(yaxis = list(title = FALSE),
         xaxis = list(title = FALSE, ticksuffix = "%",  range = c(0, 20)),
        showlegend = FALSE)
annotations = list( 
  list( 
    x = 0.2,  
...
## Which content rating has more paid apps? 
```{r echo=FALSE, warning=FALSE, message=FALSE}
content_df <- df %>%
  select(content.rating, type) %>%
  group_by(type, content.rating) %>%
  summarise(no_apps = n()) %>%
  mutate(freq =round(no_apps / sum(no_apps), 2)) %>%
  arrange(desc(no_apps))


content_df$content.rating <- factor(content_df$content.rating, 
                                      levels = c( "Everyone 10+",
                                                  "Adults only 18+",
                                                  "Teen","Everyone"
                                                  ))
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
content_df %>% 
plot_ly(y = ~content.rating, x = ~freq*100, color = ~type,
        hovertemplate = ~paste("content.rating:", content.rating, "<br>", 
                               "Type:", type, "<br>",
                               "Number of content.rating:", no_apps, "<br>", 
                               "Percentage:", formattable::percent(freq))) %>%
add_bars(showlegend = FALSE, colors = type_colors ) %>%
layout(yaxis = list(title = FALSE),
       xaxis = list(title = "Percentage of Apps",     
        ticksuffix = "%",  range = c(0, 100)),
       title= 'What content.rating has the most apps?')
```
...
### Subplots
creating subplots in R to be displayed correctly is very difficult.  An attempt was made to make subplots out of bar chart and pie chart for content rating so that the user can easily see the difference, especially in RMD file without scrolling too much, however, the attempt did not go as planned and it was decided to abandon the plot. 
```{r echo=FALSE, warning=FALSE, message=FALSE}
content_bar <- df %>%
  count(content.rating) %>%
  mutate(content.rating = fct_reorder(content.rating, n, .desc = TRUE), freq = formattable::percent(n / sum(n), 2)) %>%
...
content_pie <- plot_ly()
content_pie <- content_pie %>% add_pie(data = count(df,content.rating), labels = ~content.rating, values = ~n, 
               textinfo='label+percent',
               insidetextorientation='horizontal',
               title = 'What Content Rating is the most common?', 
                marker = list(
...
fig <- subplot(content_bar, content_pie, nrows = 2) %>% 
  layout(title = 'What categories have more apps?', annotations = annotations)
fig
...
paid_graph <- paid_df %>%
  ...
  plot_ly(y = ~category, x = ~n, color = ~category,
          hovertemplate = ~paste("Category:", category, "<br>", 
                                 ...
                                 "Number of Category:", n, "<br>", 
                                 "Percentage:", freq)) %>%
  add_bars(showlegend = FALSE, colors = c( '#7F7F7F',  '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F',
                                           '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', 
                                            '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', 
                                            '#D62728', '#7F7F7F','#7F7F7F' ) ) %>%
  layout(yaxis = list(title = FALSE),
         xaxis = list(title = "Number of Apps"),
         title= 'What category has the most apps?')
...
free_graph <- free_df %>%
  ...
  plot_ly(y = ~category, x = ~n, color = ~category,
          hovertemplate = ~paste("Category:", category, "<br>",
                                 ...
                                 "Number of Category:", n, "<br>", 
                                 "Percentage:", freq)) %>%
  add_bars(showlegend = FALSE, colors = c(  '#7F7F7F',  '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', 
                                            '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', 
                                            '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', 
                                            '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', '#7F7F7F', 
                                            '#7F7F7F','#D62728', '#7F7F7F' ) ) %>%
  layout(yaxis = list(title = FALSE),
         xaxis = list(title = "Number of Apps"),
         title= 'What category has the most apps?')
annotations = list( 
...
## Pyramid Graph - Content Rating
```{r echo=FALSE, warning=FALSE, message=FALSE}
paid_content_df <-  paid_df %>%
  select(content.rating) %>%
  group_by(content.rating) %>%
  summarise(paid_no_apps = n()) %>%
  mutate(paid_freq = formattable::percent(paid_no_apps / sum(paid_no_apps), 2))
  # arrange(desc(paid_no_apps))
...
free_content_df <-  free_df %>%
  ...
  group_by(content.rating) %>%
  summarise(free_no_apps = n()) %>%
  mutate(free_freq = formattable::percent(free_no_apps / sum(free_no_apps), 2))
#   arrange(desc(free_no_apps))
...
paid_graph <- content_df %>%
  plot_ly(y = ~content.rating, x = ~paid_freq, color = ~content.rating,
          hovertemplate = ~paste("content.rating:", content.rating, "<br>", 
                                 "Type: Paid", "<br>",
                                 "Number of content.rating:", paid_no_apps, "<br>", 
                                 "Percentage:", paid_freq)) %>%
  add_bars(showlegend = FALSE, colors = c(  '#7F7F7F',  '#7F7F7F', '#7F7F7F', '#7F7F7F' ) ) %>%
  layout(yaxis = list(title = FALSE),
         xaxis = list(title = "Number of Apps"),
         title= 'What content.rating has the most apps?')

free_graph <-  content_df %>%
  plot_ly(y = ~content.rating, x = ~free_freq, color = ~content.rating,
          hovertemplate = ~paste("content.rating:", content.rating, "<br>",
                                 "Type: Free", "<br>",
                                 "Number of content.rating:", free_no_apps, "<br>", 
                                 "Percentage:", free_freq)) %>%
  add_bars(showlegend = FALSE, colors = c(   '#7F7F7F', '#7F7F7F','#7F7F7F', '#7F7F7F' ) ) %>%
  layout(yaxis = list(title = FALSE),
         xaxis = list(title = "Number of Apps"),
         title= 'What category has the most apps?')
